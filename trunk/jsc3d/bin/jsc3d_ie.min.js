...
a.mouseUpHandler=function(b){if(this.onmouseup){var c=this.pick(b.clientX,b.clientY);this.onmouseup(c.canvasX,c.canvasY,b.button,c.depth,c.mesh)}if(this.isDefaultInputHandlerEnabled){c=this.keyStates[16]==true;if(this.buttonStates[1]==true){if(c&&b.clientY!=this.mouseY)this.zoomFactor*=this.mouseY<b.clientY?Math.pow(1.11,~~(1+(b.clientY-this.mouseY)*8/this.canvas.height)):Math.pow(0.9,~~(1+(this.mouseY-b.clientY)*8/this.canvas.height));else{c=(b.clientX-this.mouseX)*180/this.canvas.height;this.rotMatrix.rotateAboutXAxis((b.clientY-
this.mouseY)*180/this.canvas.width);this.rotMatrix.rotateAboutYAxis(c)}this.mouseX=b.clientX;this.mouseY=b.clientY;this.update()}this.buttonStates[b.button]=false}};a.mouseMoveHandler=function(b){if(this.onmousemove){var c=this.pick(b.clientX,b.clientY);this.onmousemove(c.canvasX,c.canvasY,b.button,c.depth,c.mesh)}};a.keyDownHandler=function(b){this.keyStates[b.keyCode]=true};a.keyUpHandler=function(b){this.keyStates[b.keyCode]=false};
