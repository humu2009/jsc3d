...
a.mouseMoveHandler=function(b){if(this.onmousemove){var c=this.pick(b.clientX,b.clientY);this.onmousemove(c.canvasX,c.canvasY,b.button,c.depth,c.mesh)}b.preventDefault();if(this.isDefaultInputHandlerEnabled){c=this.keyStates[16]==true;var d=this.keyStates[17]==true;if(this.buttonStates[0]==true){if(c&&this.mouseUsage=="default"||this.mouseUsage=="zoom")this.zoomFactor*=this.mouseY<=b.clientY?1.11:0.9;else if(d&&this.mouseUsage=="default"||this.mouseUsage=="pan"){c=this.definition=="low"?0.5:this.definition==
...
a.loadFromUrl=function(b){var c=this,d=new XMLHttpRequest;d.open("GET",b,true);if(JSC3D.PlatformInfo.browser=="ie"&&JSC3D.PlatformInfo.version>="10")d.responseType="blob";else d.overrideMimeType("text/plain; charset=x-user-defined");d.onreadystatechange=function(){if(this.readyState==4)if(this.status==200||this.status==0){JSC3D.console&&JSC3D.console.logInfo('Finished loading STL file "'+b+'".');if(c.onload){c.onprogress&&c.onprogress("Loading STL file ...",1);if(JSC3D.PlatformInfo.browser=="ie"&&
JSC3D.PlatformInfo.version>="10"){var e=new FileReader;e.onload=function(f){var g=new JSC3D.Scene;c.parseStl(g,f.target.result);c.onload(g)};e.readAsText(this.response,"x-user-defined")}else{e=new JSC3D.Scene;c.parseStl(e,this.responseText);c.onload(e)}}}else{JSC3D.console&&JSC3D.console.logError('Failed to load STL file "'+b+'".');c.onerror&&c.onerror('Failed to load STL file "'+b+'".')}};if(this.onprogress){this.onprogress("Loading STL file ...",0);d.onprogress=function(e){c.onprogress("Loading STL file ...",
e.position/e.totalSize)}}d.send()};a.setDecimalPrecision=function(b){this.decimalPrecision=b};
