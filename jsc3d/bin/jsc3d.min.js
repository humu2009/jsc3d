...
0;this.afterupdate=this.beforeupdate=this.onmousemove=this.onmouseup=this.onmousedown=null;this.mouseUsage="default";this.isDefaultInputHandlerEnabled=true;var d=this;if(JSC3D.PlatformInfo.isTouchDevice){this.canvas.addEventListener("touchstart",function(e){d.touchStartHandler(e)},false);this.canvas.addEventListener("touchend",function(e){d.touchEndHandler(e)},false);this.canvas.addEventListener("touchmove",function(e){d.touchMoveHandler(e)},false)}else{this.canvas.addEventListener("mousedown",function(e){d.mouseDownHandler(e)},
false);this.canvas.addEventListener("mouseup",function(e){d.mouseUpHandler(e)},false);this.canvas.addEventListener("mousemove",function(e){d.mouseMoveHandler(e)},false);document.addEventListener("keydown",function(e){d.keyDownHandler(e)},false);document.addEventListener("keyup",function(e){d.keyUpHandler(e)},false)}};a=JSC3D.Viewer.prototype;a.setParameter=function(b,c){this.params[b]=c};
...
a.mouseMoveHandler=function(b){if(this.onmousemove){var c=this.pick(b.clientX,b.clientY);this.onmousemove(c.canvasX,c.canvasY,b.button,c.depth,c.mesh)}b.preventDefault();if(this.isDefaultInputHandlerEnabled){c=this.keyStates[16]==true;var d=this.keyStates[17]==true;if(this.buttonStates[0]==true){if(c&&this.mouseUsage=="default"||this.mouseUsage=="zoom")this.zoomFactor*=this.mouseY<=b.clientY?1.04:0.96;else if(d&&this.mouseUsage=="default"||this.mouseUsage=="pan"){c=this.definition=="low"?0.5:this.definition==
"high"?2:1;this.panning[0]+=c*(b.clientX-this.mouseX);this.panning[1]+=c*(b.clientY-this.mouseY)}else if(this.mouseUsage=="default"||this.mouseUsage=="rotate"){c=(b.clientX-this.mouseX)*360/this.canvas.height;this.rotMatrix.rotateAboutXAxis((b.clientY-this.mouseY)*360/this.canvas.width);this.rotMatrix.rotateAboutYAxis(c)}this.mouseX=b.clientX;this.mouseY=b.clientY;this.update()}}};
...
a.touchMoveHandler=function(b){if(b.touches.length>0){var c=b.touches[0].clientX,d=b.touches[0].clientY;if(this.onmousemove){var e=this.pick(c,d);this.onmousemove(e.canvasX,e.canvasY,0,e.depth,e.mesh)}b.preventDefault();if(this.isDefaultInputHandlerEnabled){if(this.mouseUsage=="zoom")this.zoomFactor*=this.mouseY<=d?1.04:0.96;else if(this.mouseUsage=="pan"){b=this.definition=="low"?0.5:this.definition=="high"?2:1;this.panning[0]+=b*(c-this.mouseX);this.panning[1]+=b*(d-this.mouseY)}else if(this.mouseUsage==
"default"||this.mouseUsage=="rotate"){b=(c-this.mouseX)*360/this.canvas.height;this.rotMatrix.rotateAboutXAxis((d-this.mouseY)*360/this.canvas.width);this.rotMatrix.rotateAboutYAxis(b)}this.mouseX=c;this.mouseY=d;this.update()}}};a.keyDownHandler=function(b){if(this.isDefaultInputHandlerEnabled)this.keyStates[b.keyCode]=true};a.keyUpHandler=function(b){if(this.isDefaultInputHandlerEnabled)this.keyStates[b.keyCode]=false};
...
a.fillGradientBackground=function(){for(var b=this.frameWidth,c=this.frameHeight,d=this.bkgColorBuffer,e=(this.bkgColor1&16711680)>>16,f=(this.bkgColor1&65280)>>8,g=this.bkgColor1&255,h=(this.bkgColor2&16711680)>>16,i=(this.bkgColor2&65280)>>8,l=this.bkgColor2&255,m=0,u=0;u<c;u++)for(var j=e+u*(h-e)/c&255,B=f+u*(i-f)/c&255,o=g+u*(l-g)/c&255,M=0;M<b;M++)d[m++]=j<<16|B<<8|o};
...
a.endScene=function(){var b=this.canvasData.data,c=this.canvas.width,d=this.canvas.height,e=this.colorBuffer,f=this.frameWidth,g=f*this.frameHeight;switch(this.definition){case "low":var h=c>>1,i=f-h,l=0,m=0;for(g=0;g<d;g++){for(var u=0;u<c;u++){f=e[l];b[m]=(f&16711680)>>16;b[m+1]=(f&65280)>>8;b[m+2]=f&255;b[m+3]=255;l+=u&1;m+=4}l+=g&1?i:-h}break;case "high":for(g=m=l=0;g<d;g++){for(u=0;u<c;u++){h=e[l];i=e[l+1];var j=e[l+f],B=e[l+f+1];b[m]=(h&16711680)+(i&16711680)+(j&16711680)+(B&16711680)>>18;b[m+
1]=(h&65280)+(i&65280)+(j&65280)+(B&65280)>>10;b[m+2]=(h&255)+(i&255)+(j&255)+(B&255)>>2;b[m+3]=255;l+=2;m+=4}l+=f}break;case "standard":default:for(m=l=0;l<g;l++,m+=4){f=e[l];b[m]=(f&16711680)>>16;b[m+1]=(f&65280)>>8;b[m+2]=f&255;b[m+3]=255}break}};
...
a.renderPoint=function(b){var c=this.frameWidth,d=c-1,e=this.frameHeight-1,f=b.transformedVertexBuffer,g=this.colorBuffer,h=this.zBuffer,i=this.selectionBuffer,l=f.length/3,m=b.internalId;b=b.material?b.material.diffuseColor:this.defaultMaterial.diffuseColor;for(var u=0,j=0;u<l;u++,j+=3){var B=~~(f[j]+0.5),o=~~(f[j+1]+0.5),M=f[j+2];if(B>=0&&B<d&&o>=0&&o<e){B=o*c+B;if(M>h[B]){h[B]=M;g[B]=b;i[B]=m}B++;if(M>h[B]){h[B]=M;g[B]=b;i[B]=m}B+=d;if(M>h[B]){h[B]=M;g[B]=b;i[B]=m}B++;if(M>h[B]){h[B]=M;g[B]=b;
i[B]=m}}}};
a.renderWireframe=function(b){var c=this.frameWidth,d=c-1,e=this.frameHeight-1,f=b.indexBuffer,g=b.transformedVertexBuffer,h=b.transformedFaceNormalZBuffer,i=this.colorBuffer,l=this.zBuffer,m=this.selectionBuffer,u=b.faceCount,j=b.internalId,B=b.material?b.material.diffuseColor:this.defaultMaterial.diffuseColor;if(!h||h.length<u){b.transformedFaceNormalZBuffer=new Array(u);h=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,h);for(var o=0,M=0;o<u;){var J=
h[o++];if(b.isDoubleSided)J=J>0?J:-J;if(J<0){do;while(f[M++]!=-1)}else{var x,K;x=f[M++]*3;K=f[M++]*3;J=x;for(var H=false;!H;){var F=~~(g[x]+0.5),p=~~(g[x+1]+0.5),r=g[x+2],w=~~(g[K]+0.5),s=~~(g[K+1]+0.5),v=g[K+2],k=w-F,D=s-p,A=v-r,t,n,T;if(Math.abs(k)>Math.abs(D)){t=k;n=k>0?1:-1;T=k!=0?n*D/k:0;k=k!=0?n*A/k:0}else{t=D;T=D>0?1:-1;n=D!=0?T*k/D:0;k=D!=0?T*A/D:0}F=F;p=p;r=r;if(t<0){F=w;p=s;r=v;t=-t;n=-n;T=-T;k=-k}for(w=0;w<t;w++){if(F>=0&&F<d&&p>=0&&p<e){s=~~p*c+~~F;if(r>l[s]){l[s]=r;i[s]=B;m[s]=j}}F+=
n;p+=T;r+=k}if(K==J)H=true;else{x=K;K=f[M]!=-1?f[M++]*3:J}}M++}}};
a.renderSolidFlat=function(b){var c=this.frameWidth,d=this.frameHeight,e=b.indexBuffer,f=b.transformedVertexBuffer,g=b.transformedFaceNormalZBuffer,h=this.colorBuffer,i=this.zBuffer,l=this.selectionBuffer,m=b.faceCount,u=b.internalId,j=b.material?b.material:this.defaultMaterial,B=j.getPalette(),o=j.transparency==0,M=j.transparency*255,J=255-M;if(j.transparency!=1){if(!g||g.length<m){b.transformedFaceNormalZBuffer=new Array(m);g=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,
g);j=new Array(3);for(var x=new Array(3),K=new Array(3),H=0,F=0;H<m;){var p=g[H++];if(b.isDoubleSided)p=p>0?p:-p;if(p<0){do;while(e[F++]!=-1)}else{p=B[~~(p*255)];var r,w,s;r=e[F++]*3;w=e[F++]*3;do{s=e[F++]*3;j[0]=~~(f[r]+0.5);x[0]=~~(f[r+1]+0.5);K[0]=f[r+2];j[1]=~~(f[w]+0.5);x[1]=~~(f[w+1]+0.5);K[1]=f[w+2];j[2]=~~(f[s]+0.5);x[2]=~~(f[s+1]+0.5);K[2]=f[s+2];w=x[0]<x[1]?0:1;w=x[w]<x[2]?w:2;var v=x[0]>x[1]?0:1;v=x[v]>x[2]?v:2;var k=3-v-w;if(w!=v){var D=j[v],A=K[v],t=x[v]-x[w];t=t!=0?t:1;var n=(j[v]-j[w])/
t;t=(K[v]-K[w])/t;var T=j[v],y=K[v],E=x[v]-x[k];E=E!=0?E:1;var C=(j[v]-j[k])/E;E=(K[v]-K[k])/E;var q=j[k],la=K[k],z=x[k]-x[w];z=z!=0?z:1;var ra=(j[k]-j[w])/z;z=(K[k]-K[w])/z;var ma=x[v]*c;for(v=x[v];v>x[w];v--){if(v>=0&&v<d){var Q=~~D,V=A,G,sa;if(v>x[k]){G=~~T;sa=y}else{G=~~q;sa=la}if(Q>G){var da;da=Q;Q=G;G=da;da=V;V=sa;sa=da}if(Q<0)Q=0;if(G>=c)G=c-1;sa=Q!=G?(sa-V)/(G-Q):1;da=ma+Q;if(o){Q=Q;for(V=V;Q<=G;Q++,V+=sa){if(V>i[da]){i[da]=V;h[da]=p;l[da]=u}da++}}else{Q=Q;for(V=V;Q<G;Q++,V+=sa){if(V>i[da]){var ya=
h[da];h[da]=(ya&16711680)*M+(p&16711680)*J>>8&16711680|(ya&65280)*M+(p&65280)*J>>8&65280|(ya&255)*M+(p&255)*J>>8&255;l[da]=u}da++}}}D-=n;A-=t;if(v>x[k]){T-=C;y-=E}else{q-=ra;la-=z}ma-=c}}w=s}while(e[F]!=-1);F++}}}};
a.renderSolidSmooth=function(b){var c=this.frameWidth,d=this.frameHeight,e=b.indexBuffer,f=b.transformedVertexBuffer,g=b.transformedVertexNormalZBuffer,h=b.vertexNormalIndexBuffer?b.vertexNormalIndexBuffer:b.indexBuffer,i=b.transformedFaceNormalZBuffer,l=this.colorBuffer,m=this.zBuffer,u=this.selectionBuffer,j=b.faceCount,B=b.internalId,o=b.material?b.material:this.defaultMaterial,M=o.getPalette(),J=o.transparency==0,x=o.transparency*255,K=255-x;if(o.transparency!=1){if(!g||g.length<b.vertexNormalBuffer.length/
3){b.transformedVertexNormalZBuffer=new Array(b.vertexNormalBuffer.length/3);g=b.transformedVertexNormalZBuffer}if(!i||i.length<j){b.transformedFaceNormalZBuffer=new Array(j);i=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.vertexNormalBuffer,g);JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,i);b=b.isDoubleSided;o=new Array(3);for(var H=new Array(3),F=new Array(3),p=new Array(3),r=0,w=0;r<j;){var s=i[r++];if(b)s=s>0?s:-s;if(s<0){do;while(e[w++]!=-1)
}else{var v,k,D,A,t;s=e[w]*3;D=h[w];w++;k=e[w]*3;A=h[w];w++;do{v=e[w];v=v*3;t=h[w];w++;o[0]=~~(f[s]+0.5);H[0]=~~(f[s+1]+0.5);F[0]=f[s+2];o[1]=~~(f[k]+0.5);H[1]=~~(f[k+1]+0.5);F[1]=f[k+2];o[2]=~~(f[v]+0.5);H[2]=~~(f[v+1]+0.5);F[2]=f[v+2];p[0]=g[D];p[1]=g[A];p[2]=g[t];if(b){if(p[0]<0)p[0]=-p[0];if(p[1]<0)p[1]=-p[1];if(p[2]<0)p[2]=-p[2]}k=H[0]<H[1]?0:1;k=H[k]<H[2]?k:2;var n=H[0]>H[1]?0:1;n=H[n]>H[2]?n:2;A=3-n-k;if(k!=n){var T=o[n],y=F[n],E=p[n]*255,C=H[n]-H[k];C=C!=0?C:1;var q=(o[n]-o[k])/C,la=(F[n]-
F[k])/C;C=(p[n]-p[k])*255/C;var z=o[n],ra=F[n],ma=p[n]*255,Q=H[n]-H[A];Q=Q!=0?Q:1;var V=(o[n]-o[A])/Q,G=(F[n]-F[A])/Q;Q=(p[n]-p[A])*255/Q;var sa=o[A],da=F[A],ya=p[A]*255,ta=H[A]-H[k];ta=ta!=0?ta:1;var ea=(o[A]-o[k])/ta,Ea=(F[A]-F[k])/ta;ta=(p[A]-p[k])*255/ta;var Da=H[n]*c;for(n=H[n];n>H[k];n--){if(n>=0&&n<d){var S=~~T,Y=y,na=E,oa,ua,ja;if(n>H[A]){oa=~~z;ua=ra;ja=ma}else{oa=~~sa;ua=da;ja=ya}if(S>oa){var fa;fa=S;S=oa;oa=fa;fa=Y;Y=ua;ua=fa;fa=na;na=ja;ja=fa}ua=S!=oa?(ua-Y)/(oa-S):1;ja=S!=oa?(ja-na)/
(oa-S):1;if(S<0){Y-=S*ua;na-=S*ja;S=0}if(oa>=c)oa=c-1;fa=Da+S;if(J){S=S;Y=Y;for(na=na;S<=oa;S++,Y+=ua,na+=ja){if(Y>m[fa]){m[fa]=Y;l[fa]=M[na>0?~~na:0];u[fa]=B}fa++}}else{S=S;Y=Y;for(na=na;S<oa;S++,Y+=ua,na+=ja){if(Y>m[fa]){var pa=M[na>0?~~na:0],va=l[fa];l[fa]=(va&16711680)*x+(pa&16711680)*K>>8&16711680|(va&65280)*x+(pa&65280)*K>>8&65280|(va&255)*x+(pa&255)*K>>8&255;u[fa]=B}fa++}}}T-=q;y-=la;E-=C;if(n>H[A]){z-=V;ra-=G;ma-=Q}else{sa-=ea;da-=Ea;ya-=ta}Da-=c}}k=v;A=t}while(e[w]!=-1);w++}}}};
a.renderSolidTexture=function(b){var c=this.frameWidth,d=this.frameHeight,e=b.indexBuffer,f=b.transformedVertexBuffer,g=b.transformedFaceNormalZBuffer,h=this.colorBuffer,i=this.zBuffer,l=this.selectionBuffer,m=b.faceCount,u=b.internalId,j=b.texture,B=!j.hasTransparency,o=b.texCoordBuffer,M=b.texCoordIndexBuffer?b.texCoordIndexBuffer:b.indexBuffer,J=j.data,x=j.width,K=x-1,H=j.hasMipmap()?j.mipmaps:null,F=H?j.mipentries:null;if(!g||g.length<m){b.transformedFaceNormalZBuffer=new Array(m);g=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,
b.faceNormalBuffer,g);for(var p=new Array(3),r=new Array(3),w=new Array(3),s=new Array(3),v=new Array(3),k=0,D=0;k<m;){var A=g[k++];if(b.isDoubleSided)A=A>0?A:-A;if(A<0){do;while(e[D++]!=-1)}else{var t,n,T,y,E;A=e[D]*3;T=M[D]*2;D++;t=e[D]*3;y=M[D]*2;D++;if(H){n=e[D]*3;E=M[D]*2;x=j.width;p[0]=f[A];r[0]=f[A+1];p[1]=f[t];r[1]=f[t+1];p[2]=f[n];r[2]=f[n+1];s[0]=o[T]*x;v[0]=o[T+1]*x;s[1]=o[y]*x;v[1]=o[y+1]*x;s[2]=o[E]*x;v[2]=o[E+1]*x;J=(p[1]-p[0])*(r[2]-r[0])-(r[1]-r[0])*(p[2]-p[0]);if(J<0)J=-J;J+=1;K=
(s[1]-s[0])*(v[2]-v[0])-(v[1]-v[0])*(s[2]-s[0]);if(K<0)K=-K;J=K/J;K=0;if(J<F[1])K=0;else if(J>=F[F.length-1]){K=F.length-1;x=1}else for(;J>=F[K+1];){K++;x/=2}J=H[K];K=x-1}do{n=e[D]*3;E=M[D]*2;D++;p[0]=~~(f[A]+0.5);r[0]=~~(f[A+1]+0.5);w[0]=f[A+2];p[1]=~~(f[t]+0.5);r[1]=~~(f[t+1]+0.5);w[1]=f[t+2];p[2]=~~(f[n]+0.5);r[2]=~~(f[n+1]+0.5);w[2]=f[n+2];s[0]=o[T]*x;v[0]=o[T+1]*x;s[1]=o[y]*x;v[1]=o[y+1]*x;s[2]=o[E]*x;v[2]=o[E+1]*x;t=r[0]<r[1]?0:1;t=r[t]<r[2]?t:2;var C=r[0]>r[1]?0:1;C=r[C]>r[2]?C:2;y=3-C-t;if(t!=
...
24&255,X=255-P;h[L]=(ca&16711680)*X+(Z&16711680)*P>>8&16711680|(ca&65280)*X+(Z&65280)*P>>8&65280|(ca&255)*X+(Z&255)*P>>8&255;l[L]=u}L++}}}q-=Q;la-=V;z-=G;ra-=ma;if(C>r[y]){sa-=Ea;da-=Da;ya-=S;ta-=ea}else{Y-=fa;na-=pa;oa-=va;ua-=ja}Fa-=c}}t=n;y=E}while(e[D]!=-1);D++}}};
a.renderTextureFlat=function(b){var c=this.frameWidth,d=this.frameHeight,e=b.indexBuffer,f=b.transformedVertexBuffer,g=b.transformedFaceNormalZBuffer,h=this.colorBuffer,i=this.zBuffer,l=this.selectionBuffer,m=b.faceCount,u=b.internalId,j=b.material?b.material:this.defaultMaterial,B=j.getPalette(),o=b.texture,M=j.transparency==0&&!o.hasTransparency,J=~~((1-j.transparency)*255),x=b.texCoordBuffer,K=b.texCoordIndexBuffer?b.texCoordIndexBuffer:b.indexBuffer,H=o.data,F=o.width,p=F-1,r=o.hasMipmap()?o.mipmaps:
null,w=r?o.mipentries:null;if(j.transparency!=1){if(!g||g.length<m){b.transformedFaceNormalZBuffer=new Array(m);g=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,g);j=new Array(3);for(var s=new Array(3),v=new Array(3),k=new Array(3),D=new Array(3),A=0,t=0;A<m;){var n=g[A++];if(b.isDoubleSided)n=n>0?n:-n;if(n<0){do;while(e[t++]!=-1)}else{n=B[~~(n*255)];var T,y,E,C,q,la;T=e[t]*3;C=K[t]*2;t++;y=e[t]*3;q=K[t]*2;t++;if(r){E=e[t]*3;la=K[t]*2;F=o.width;j[0]=
...
ha+=ca){if(U>i[P]){i[P]=U;var X=H[(ha&p)*F+(ka&p)],N=((n&16711680)>>16)*((X&16711680)>>8),ba=((n&65280)>>8)*((X&65280)>>8),W=(n&255)*(X&255)>>8;h[P]=N&16711680|ba&65280|W&255;l[P]=u}P++}}else{O=O;U=U;ka=ka;for(ha=ha;O<I;O++,U+=L,ka+=Z,ha+=ca){if(U>i[P]){W=H[(ha&p)*F+(ka&p)];X=h[P];var ia=(W>>24&255)*(J&255)>>8;N=((n&16711680)>>16)*((W&16711680)>>8);ba=((n&65280)>>8)*((W&65280)>>8);W=(n&255)*(W&255)>>8;if(ia>250)i[P]=U;else{var R=255-ia;N=N*ia+(X&16711680)*R>>8;ba=ba*ia+(X&65280)*R>>8;W=W*ia+(X&255)*
...
a.renderTextureSmooth=function(b){var c=this.frameWidth,d=this.frameHeight,e=b.indexBuffer,f=b.transformedVertexBuffer,g=b.transformedVertexNormalZBuffer,h=b.vertexNormalIndexBuffer?b.vertexNormalIndexBuffer:b.indexBuffer,i=b.transformedFaceNormalZBuffer,l=this.colorBuffer,m=this.zBuffer,u=this.selectionBuffer,j=b.faceCount,B=b.internalId,o=b.material?b.material:this.defaultMaterial,M=o.getPalette(),J=b.texture,x=o.transparency==0&&!J.hasTransparency,K=~~((1-o.transparency)*255),H=b.texCoordBuffer,
F=b.texCoordIndexBuffer?b.texCoordIndexBuffer:b.indexBuffer,p=J.data,r=J.width,w=r-1,s=J.hasMipmap()?J.mipmaps:null,v=s?J.mipentries:null;if(o.transparency!=1){if(!g||g.length<b.vertexNormalBuffer.length/3){b.transformedVertexNormalZBuffer=new Array(b.vertexNormalBuffer.length/3);g=b.transformedVertexNormalZBuffer}if(!i||i.length<j){b.transformedFaceNormalZBuffer=new Array(j);i=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.vertexNormalBuffer,g);JSC3D.Math3D.transformVectorZs(this.rotMatrix,
b.faceNormalBuffer,i);b=b.isDoubleSided;o=new Array(3);for(var k=new Array(3),D=new Array(3),A=new Array(3),t=new Array(3),n=new Array(3),T=0,y=0;T<j;){var E=i[T++];if(b)E=E>0?E:-E;if(E<0){do;while(e[y++]!=-1)}else{var C,q,la,z,ra,ma,Q,V;E=e[y]*3;la=F[y]*2;ma=h[y];y++;q=e[y]*3;z=F[y]*2;Q=h[y];y++;if(s){C=e[y]*3;ra=F[y]*2;r=J.width;o[0]=f[E];k[0]=f[E+1];o[1]=f[q];k[1]=f[q+1];o[2]=f[C];k[2]=f[C+1];t[0]=H[la]*r;n[0]=H[la+1]*r;t[1]=H[z]*r;n[1]=H[z+1]*r;t[2]=H[ra]*r;n[2]=H[ra+1]*r;p=(o[1]-o[0])*(k[2]-
k[0])-(k[1]-k[0])*(o[2]-o[0]);if(p<0)p=-p;p+=1;w=(t[1]-t[0])*(n[2]-n[0])-(n[1]-n[0])*(t[2]-t[0]);if(w<0)w=-w;p=w/p;w=0;if(p<v[1])w=0;else if(p>=v[v.length-1]){w=v.length-1;r=1}else for(;p>=v[w+1];){w++;r/=2}p=s[w];w=r-1}do{C=e[y];C=C*3;ra=F[y]*2;V=h[y];y++;o[0]=~~(f[E]+0.5);k[0]=~~(f[E+1]+0.5);D[0]=f[E+2];o[1]=~~(f[q]+0.5);k[1]=~~(f[q+1]+0.5);D[1]=f[q+2];o[2]=~~(f[C]+0.5);k[2]=~~(f[C+1]+0.5);D[2]=f[C+2];t[0]=H[la]*r;n[0]=H[la+1]*r;t[1]=H[z]*r;n[1]=H[z+1]*r;t[2]=H[ra]*r;n[2]=H[ra+1]*r;A[0]=g[ma];A[1]=
g[Q];A[2]=g[V];if(b){if(A[0]<0)A[0]=-A[0];if(A[1]<0)A[1]=-A[1];if(A[2]<0)A[2]=-A[2]}q=k[0]<k[1]?0:1;q=k[q]<k[2]?q:2;var G=k[0]>k[1]?0:1;G=k[G]>k[2]?G:2;z=3-G-q;if(q!=G){Q=o[G];var sa=D[G],da=t[G],ya=n[G],ta=A[G]*255,ea=k[G]-k[q];ea=ea!=0?ea:1;var Ea=(o[G]-o[q])/ea,Da=(D[G]-D[q])/ea,S=(t[G]-t[q])/ea,Y=(n[G]-n[q])/ea;ea=(A[G]-A[q])*255/ea;var na=o[G],oa=D[G],ua=t[G],ja=n[G],fa=A[G]*255,pa=k[G]-k[z];pa=pa!=0?pa:1;var va=(o[G]-o[z])/pa,Fa=(D[G]-D[z])/pa,aa=(t[G]-t[z])/pa,ga=(n[G]-n[z])/pa;pa=(A[G]-A[z])*
255/pa;var wa=o[z],O=D[z],U=t[z],ka=n[z],ha=A[z]*255,I=k[z]-k[q];I=I!=0?I:1;var L=(o[z]-o[q])/I,Z=(D[z]-D[q])/I,ca=(t[z]-t[q])/I,P=(n[z]-n[q])/I;I=(A[z]-A[q])*255/I;var X=k[G]*c;for(G=k[G];G>k[q];G--){if(G>=0&&G<d){var N=~~Q,ba=sa,W=da,ia=ya,R=ta,$,xa,za,Aa,Ba;if(G>k[z]){$=~~na;xa=oa;za=ua;Aa=ja;Ba=fa}else{$=~~wa;xa=O;za=U;Aa=ka;Ba=ha}if(N>$){var qa;qa=N;N=$;$=qa;qa=ba;ba=xa;xa=qa;qa=W;W=za;za=qa;qa=ia;ia=Aa;Aa=qa;qa=R;R=Ba;Ba=qa}xa=N!=$?(xa-ba)/($-N):1;za=N!=$?(za-W)/($-N):1;Aa=N!=$?(Aa-ia)/($-N):
1;Ba=N!=$?(Ba-R)/($-N):0;if(N<0){ba-=N*xa;W-=N*za;ia-=N*Aa;R-=N*Ba;N=0}if($>=c)$=c-1;qa=X+N;if(x){N=N;ba=ba;R=R;W=W;for(ia=ia;N<=$;N++,ba+=xa,R+=Ba,W+=za,ia+=Aa){if(ba>m[qa]){m[qa]=ba;var Ca=M[R>0?~~R:0],Ha=p[(ia&w)*r+(W&w)],Ia=((Ca&16711680)>>16)*((Ha&16711680)>>8),Ja=((Ca&65280)>>8)*((Ha&65280)>>8);Ca=(Ca&255)*(Ha&255)>>8;l[qa]=Ia&16711680|Ja&65280|Ca&255;u[qa]=B}qa++}}else{N=N;ba=ba;R=R;W=W;for(ia=ia;N<$;N++,ba+=xa,R+=Ba,W+=za,ia+=Aa){if(ba>m[qa]){Ca=M[R>0?~~R:0];var Ga=p[(ia&w)*r+(W&w)];Ha=l[qa];
var Ka=(Ga>>24&255)*(K&255)>>8;Ia=((Ca&16711680)>>16)*((Ga&16711680)>>8);Ja=((Ca&65280)>>8)*((Ga&65280)>>8);Ca=(Ca&255)*(Ga&255)>>8;if(Ka>250)m[qa]=ba;else{Ga=255-Ka;Ia=Ia*Ka+(Ha&16711680)*Ga>>8;Ja=Ja*Ka+(Ha&65280)*Ga>>8;Ca=Ca*Ka+(Ha&255)*Ga>>8}l[qa]=Ia&16711680|Ja&65280|Ca&255;u[qa]=B}qa++}}}Q-=Ea;sa-=Da;da-=S;ya-=Y;ta-=ea;if(G>k[z]){na-=va;oa-=Fa;ua-=aa;ja-=ga;fa-=pa}else{wa-=L;O-=Z;U-=ca;ka-=P;ha-=I}X-=c}}q=C;z=ra;Q=V}while(e[y]!=-1);y++}}}};
a.renderSolidSphereMapped=function(b){var c=this.frameWidth,d=this.frameHeight,e=b.indexBuffer,f=b.transformedVertexBuffer,g=b.transformedVertexNormalBuffer,h=b.vertexNormalIndexBuffer?b.vertexNormalIndexBuffer:b.indexBuffer,i=b.transformedFaceNormalZBuffer,l=this.colorBuffer,m=this.zBuffer,u=this.selectionBuffer,j=b.faceCount,B=b.internalId,o=b.material?b.material:this.defaultMaterial,M=o.getPalette(),J=this.sphereMap,x=J.data;J=J.width;var K=J-1,H=o.transparency==0,F=o.transparency*255,p=255-F;
if(o.transparency!=1){if(!g||g.length<b.vertexNormalBuffer.length){b.transformedVertexNormalBuffer=new Array(b.vertexNormalBuffer.length);g=b.transformedVertexNormalBuffer}if(!i||i.length<j){b.transformedFaceNormalZBuffer=new Array(j);i=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectors(this.rotMatrix,b.vertexNormalBuffer,g);JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,i);b=b.isDoubleSided;o=new Array(3);for(var r=new Array(3),w=new Array(3),s=new Array(3),v=new Array(3),
k=new Array(3),D=0,A=0;D<j;){var t=i[D++];if(b)t=t>0?t:-t;if(t<0){do;while(e[A++]!=-1)}else{var n,T,y,E,C;t=e[A]*3;y=h[A]*3;A++;n=e[A]*3;E=h[A]*3;A++;do{T=e[A]*3;C=h[A]*3;A++;o[0]=~~(f[t]+0.5);r[0]=~~(f[t+1]+0.5);w[0]=f[t+2];o[1]=~~(f[n]+0.5);r[1]=~~(f[n+1]+0.5);w[1]=f[n+2];o[2]=~~(f[T]+0.5);r[2]=~~(f[T+1]+0.5);w[2]=f[T+2];s[0]=g[y];v[0]=g[y+1];k[0]=g[y+2];s[1]=g[E];v[1]=g[E+1];k[1]=g[E+2];s[2]=g[C];v[2]=g[C+1];k[2]=g[C+2];if(b){if(k[0]<0)k[0]=-k[0];if(k[1]<0)k[1]=-k[1];if(k[2]<0)k[2]=-k[2]}n=r[0]<
r[1]?0:1;n=r[n]<r[2]?n:2;var q=r[0]>r[1]?0:1;q=r[q]>r[2]?q:2;E=3-q-n;if(n!=q){var la=o[q],z=w[q],ra=k[q]*255,ma=(s[q]/2+0.5)*J&K,Q=(0.5-v[q]/2)*J&K,V=r[q]-r[n];V=V!=0?V:1;var G=(o[q]-o[n])/V,sa=(w[q]-w[n])/V,da=(k[q]-k[n])*255/V,ya=(s[q]-s[n])/2*J/V;V=(v[n]-v[q])/2*J/V;var ta=o[q],ea=w[q],Ea=k[q]*255,Da=(s[q]/2+0.5)*J&K,S=(0.5-v[q]/2)*J&K,Y=r[q]-r[E];Y=Y!=0?Y:1;var na=(o[q]-o[E])/Y,oa=(w[q]-w[E])/Y,ua=(k[q]-k[E])*255/Y,ja=(s[q]-s[E])/2*J/Y;Y=(v[E]-v[q])/2*J/Y;var fa=o[E],pa=w[E],va=k[E]*255,Fa=(s[E]/
2+0.5)*J&K,aa=(0.5-v[E]/2)*J&K,ga=r[E]-r[n];ga=ga!=0?ga:1;var wa=(o[E]-o[n])/ga,O=(w[E]-w[n])/ga,U=(k[E]-k[n])*255/ga,ka=(s[E]-s[n])/2*J/ga;ga=(v[n]-v[E])/2*J/ga;var ha=r[q]*c;for(q=r[q];q>r[n];q--){if(q>=0&&q<d){var I=~~la,L=z,Z=ra,ca=ma,P=Q,X,N,ba,W,ia;if(q>r[E]){X=~~ta;N=ea;ba=Ea;W=Da;ia=S}else{X=~~fa;N=pa;ba=va;W=Fa;ia=aa}if(I>X){var R;R=I;I=X;X=R;R=L;L=N;N=R;R=Z;Z=ba;ba=R;R=ca;ca=W;W=R;R=P;P=ia;ia=R}N=I!=X?(N-L)/(X-I):1;ba=I!=X?(ba-Z)/(X-I):1;W=I!=X?(W-ca)/(X-I):1;ia=I!=X?(ia-P)/(X-I):1;if(I<
0){L-=I*N;Z-=I*ba;ca-=ca*W;P-=P*ia;I=0}if(X>=c)X=c-1;R=ha+I;if(H){I=I;L=L;Z=Z;ca=ca;for(P=P;I<=X;I++,L+=N,Z+=ba,ca+=W,P+=ia){if(L>m[R]){m[R]=L;var $=M[Z>0?~~Z:0],xa=x[(P&K)*J+(ca&K)],za=(($&16711680)>>16)*((xa&16711680)>>8),Aa=(($&65280)>>8)*((xa&65280)>>8);$=($&255)*(xa&255)>>8;l[R]=za&16711680|Aa&65280|$&255;u[R]=B}R++}}else{I=I;L=L;Z=Z;ca=ca;for(P=P;I<X;I++,L+=N,Z+=ba,ca+=W,P+=ia){if(L>m[R]){$=M[Z>0?~~Z:0];var Ba=x[(P&K)*J+(ca&K)];xa=l[R];za=(($&16711680)>>16)*((Ba&16711680)>>8);Aa=(($&65280)>>
8)*((Ba&65280)>>8);$=($&255)*(Ba&255)>>8;za=za*p+(xa&16711680)*F>>8;Aa=Aa*p+(xa&65280)*F>>8;$=$*p+(xa&255)*F>>8;l[R]=za&16711680|Aa&65280|$&255;u[R]=B}R++}}}la-=G;z-=sa;ra-=da;ma-=ya;Q-=V;if(q>r[E]){ta-=na;ea-=oa;Ea-=ua;Da-=ja;S-=Y}else{fa-=wa;pa-=O;va-=U;Fa-=ka;aa-=ga}ha-=c}}n=T;E=C}while(e[A]!=-1);A++}}}};a.params=null;a.canvas=null;a.ctx=null;a.canvasData=null;a.bkgColorBuffer=null;a.colorBuffer=null;a.zBuffer=null;a.selectionBuffer=null;a.frameWidth=0;a.frameHeight=0;a.scene=null;
...
a.onmouseup=null;a.onmousemove=null;a.beforeupdate=null;a.afterupdate=null;a.mouseUsage="default";a.isDefaultInputHandlerEnabled=false;JSC3D.PickInfo=function(){this.canvasY=this.canvasX=0;this.depth=-Infinity;this.mesh=null};JSC3D.Scene=function(b){this.name=b||"";this.aabb=null;this.children=[];this.maxChildId=1};a=JSC3D.Scene.prototype;a.init=function(){if(!this.isEmpty()){for(var b=0;b<this.children.length;b++)this.children[b].init();if(!this.aabb){this.aabb=new JSC3D.AABB;this.calcAABB()}}};
a.isEmpty=function(){return this.children.length==0};a.addChild=function(b){b.internalId=this.maxChildId++;this.children.push(b)};a.removeChild=function(b){for(var c=0;c<this.children.length;c++)if(this.children[c]==b){this.children.splice(c,1);break}};a.getChildren=function(){return this.children.slice(0)};a.forEachChild=function(b){if(typeof b=="function")for(var c=0;c<this.children.length;c++)if(b.call(null,this.children[c]))break};
...
JSC3D.Mesh=function(b,c,d,e,f,g,h,i,l,m,u){this.name=b||"";this.metadata="";this.visible=c!=undefined?c:true;this.aabb=null;this.vertexBuffer=i||null;this.indexBuffer=l||null;this.faceNormalBuffer=this.vertexNormalIndexBuffer=this.vertexNormalBuffer=null;this.material=d||null;this.texture=e||null;this.faceCount=0;this.creaseAngle=f>=0?f:-180;this.isDoubleSided=g||false;this.isEnvironmentCast=h||false;this.internalId=0;this.texCoordBuffer=m||null;this.texCoordIndexBuffer=u||null;this.transformedVertexNormalBuffer=
...
for(e=0;e<d.length;e++)d[e]=0;for(var l=this.vertexNormalIndexBuffer=[],m=Math.cos(this.creaseAngle*Math.PI/180),u=e=0,j=0;e<b.length;e++){g=b[e];if(g>=0){var B=u*3;h=j*3;d[B]+=f[h];d[B+1]+=f[h+1];d[B+2]+=f[h+2];var o=i[h],M=i[h+1],J=i[h+2];h=c[g];for(g=0;g<h.length;g++){var x=h[g];if(j!=x){x=x*3;if(o*i[x]+M*i[x+1]+J*i[x+2]>m){d[B]+=f[x];d[B+1]+=f[x+1];d[B+2]+=f[x+2]}}}l.push(u++)}else{j++;l.push(-1)}}JSC3D.Math3D.normalizeVectors(d,d)};a.checkValid=function(){};a.name="";a.metadata="";
...
a.generatePalette=function(){var b=(this.ambientColor&16711680)>>16,c=(this.ambientColor&65280)>>8,d=this.ambientColor&255,e=(this.diffuseColor&16711680)>>16,f=(this.diffuseColor&65280)>>8,g=this.diffuseColor&255;if(this.simulateSpecular){for(var h=0;h<204;){var i=b+h*e/204,l=c+h*f/204,m=d+h*g/204;if(i>255)i=255;if(l>255)l=255;if(m>255)m=255;this.palette[h++]=i<<16|l<<8|m}for(;h<256;){i=b+e+(h-204)*(255-e)/82;l=c+f+(h-204)*(255-f)/82;m=d+g+(h-204)*(255-g)/82;if(i>255)i=255;if(l>255)l=255;if(m>255)m=
255;this.palette[h++]=i<<16|l<<8|m}}else for(h=0;h<256;){i=b+h*e/256;l=c+h*f/256;m=d+h*g/256;if(i>255)i=255;if(l>255)l=255;if(m>255)m=255;this.palette[h++]=i<<16|l<<8|m}};a.name="";a.ambientColor=0;a.diffuseColor=8355711;a.transparency=0;a.simulateSpecular=false;a.palette=null;JSC3D.Texture=function(b,c){this.name=b||"";this.height=this.width=0;this.mipentries=this.mipmaps=this.data=null;this.hasTransparency=false;this.srcUrl="";this.onready=c&&typeof c=="function"?c:null};a=JSC3D.Texture.prototype;
...
a.createFromImage=function(b,c){if(!(b.width<=0||b.height<=0)){var d=false,e=JSC3D.Texture.cv;if(!e)try{e=document.createElement("canvas");JSC3D.Texture.cv=e;d=true}catch(f){return}var g=b.width>b.height?b.width:b.height;g=g<=32?32:g<=64?64:g<=128?128:g<=256?256:512;if(e.width!=g||e.height!=g){e.width=e.height=g;d=true}var h;try{var i=e.getContext("2d");d||i.clearRect(0,0,g,g);i.drawImage(b,0,0,g,g);h=i.getImageData(0,0,g,g).data}catch(l){return}d=h.length/4;this.data=new Array(d);for(var m=i=0;i<
d;i++,m+=4){e=h[m+3];this.data[i]=e<<24|h[m]<<16|h[m+1]<<8|h[m+2];if(e<255)this.hasTransparency=true}this.height=this.width=g;this.mipmaps=null;c&&this.generateMipmaps();this.srcUrl=b.src;this.onready!=null&&typeof this.onready=="function"&&this.onready()}};a.hasData=function(){return this.data!=null};
a.generateMipmaps=function(){if(!(this.width<=1||this.data==null||this.mipmaps!=null)){this.mipmaps=[this.data];this.mipentries=[1];for(var b=1+~~(0.1+Math.log(this.width)*Math.LOG2E),c=this.width>>1,d=1;d<b;d++){for(var e=new Array(c*c),f=this.mipmaps[d-1],g=c<<1,h=0,i=0,l=0;l<c;l++){for(var m=0;m<c;m++){var u=f[h],j=f[h+1],B=f[h+g],o=f[h+g+1];e[i]=(((u&4278190080)>>>2)+((j&4278190080)>>>2)+((B&4278190080)>>>2)+((o&4278190080)>>>2)&4278190080)+((u&16711680)+(j&16711680)+(B&16711680)+(o&16711680)>>
2&16711680)+((u&65280)+(j&65280)+(B&65280)+(o&65280)>>2&65280)+((u&255)+(j&255)+(B&255)+(o&255)>>2&255);h+=2;i++}h+=g}this.mipmaps.push(e);this.mipentries.push(Math.pow(4,d));c>>=1}}};a.hasMipmap=function(){return this.mipmaps!=null};a.name="";a.data=null;a.mipmaps=null;a.mipentries=null;a.width=0;a.height=0;a.hasTransparency=false;a.srcUrl="";a.onready=null;JSC3D.Texture.cv=null;JSC3D.AABB=function(){this.minZ=this.maxZ=this.minY=this.maxY=this.minX=this.maxX=0};
...
a.multiply=function(b){var c=b.m00*this.m01+b.m01*this.m11+b.m02*this.m21,d=b.m00*this.m02+b.m01*this.m12+b.m02*this.m22,e=b.m00*this.m03+b.m01*this.m13+b.m02*this.m23+b.m03,f=b.m10*this.m00+b.m11*this.m10+b.m12*this.m20,g=b.m10*this.m01+b.m11*this.m11+b.m12*this.m21,h=b.m10*this.m02+b.m11*this.m12+b.m12*this.m22,i=b.m10*this.m03+b.m11*this.m13+b.m12*this.m23+b.m13,l=b.m20*this.m00+b.m21*this.m10+b.m22*this.m20,m=b.m20*this.m01+b.m21*this.m11+b.m22*this.m21,u=b.m20*this.m02+b.m21*this.m12+b.m22*this.m22,
j=b.m20*this.m03+b.m21*this.m13+b.m22*this.m23+b.m23;this.m00=b.m00*this.m00+b.m01*this.m10+b.m02*this.m20;this.m01=c;this.m02=d;this.m03=e;this.m10=f;this.m11=g;this.m12=h;this.m13=i;this.m20=l;this.m21=m;this.m22=u;this.m23=j};
...
a.loadMtlFile=function(b,c,d){var e=c+d,f=this,g=new XMLHttpRequest;g.open("GET",e,true);g.onreadystatechange=function(){if(this.readyState==4){if(this.status==200||this.status==0){f.onprogress&&f.onprogress("Loading mtl file ...",1);JSC3D.console&&JSC3D.console.logInfo('Finished loading mtl file "'+e+'".');for(var h=f.parseMtl(this.responseText),i={},l=b.getChildren(),m=0;m<l.length;m++){var u=l[m];if(u.mtl!=null&&u.mtllib!=null&&u.mtllib==d){var j=h[u.mtl];if(j!=null){j.material!=null&&u.setMaterial(j.material);
...
a.parseObj=function(b,c){var d={},e=[],f=0,g=null,h="",i="",l=[],m=[];g="obj-"+f++;var u=new JSC3D.Mesh;u.name=g;u.indexBuffer=[];g=d.nomtl=u;var j=c.split(/[ \t]*\r?\n[ \t]*/);for(c=0;c<j.length;c++){i=j[c].split(/[ \t]+/);if(i.length>0)switch(i[0]){case "v":if(i.length>3)for(var B=1;B<4;B++)l.push(parseFloat(i[B]));break;case "vn":break;case "vt":if(i.length>2){m.push(parseFloat(i[1]));m.push(1-parseFloat(i[2]))}break;case "f":if(i.length>3){for(B=1;B<i.length;B++){var o=i[B].split("/");g.indexBuffer.push(parseInt(o[0])-
1);if(o.length>1&&o[1]!=""){if(!g.texCoordIndexBuffer)g.texCoordIndexBuffer=[];g.texCoordIndexBuffer.push(parseInt(o[1])-1)}}g.indexBuffer.push(-1);g.texCoordIndexBuffer&&g.texCoordIndexBuffer.push(-1)}break;case "mtllib":if(i.length>1){h=i[1];e.push(h)}else h="";break;case "usemtl":if(i.length>1&&i[1]!=""&&h!=""){i=i[1];B=h+"-"+i;g=d[B];if(!g){g=new JSC3D.Mesh;g.name="obj-"+f++;g.indexBuffer=[];g.mtllib=h;g.mtl=i;d[B]=g}g=g}else g=u;break;case "#":default:break}}f=l.length>=3?new Array(l.length/
3):null;h=m.length>=2?new Array(m.length/2):null;for(var M in d){g=d[M];if(l.length>=3&&g.indexBuffer.length>0){for(c=0;c<f.length;c++)f[c]=-1;g.vertexBuffer=[];for(c=j=u=0;c<g.indexBuffer.length;c++){u=g.indexBuffer[c];if(u!=-1)if(f[u]==-1){i=u*3;g.vertexBuffer.push(l[i]);g.vertexBuffer.push(l[i+1]);g.vertexBuffer.push(l[i+2]);g.indexBuffer[c]=j;f[u]=j;j++}else g.indexBuffer[c]=f[u]}}if(m.length>=2&&g.texCoordIndexBuffer!=null&&g.texCoordIndexBuffer.length>0){for(c=0;c<h.length;c++)h[c]=-1;g.texCoordBuffer=
[];for(c=j=u=0;c<g.texCoordIndexBuffer.length;c++){u=g.texCoordIndexBuffer[c];if(u!=-1)if(h[u]==-1){i=u*2;g.texCoordBuffer.push(m[i]);g.texCoordBuffer.push(m[i+1]);g.texCoordIndexBuffer[c]=j;h[u]=j;j++}else g.texCoordIndexBuffer[c]=h[u]}}g.isTrivial()||b.addChild(g)}return e};
...
a.loadFromUrl=function(b){var c=this,d=JSC3D.PlatformInfo.browser=="ie",e=d&&parseInt(JSC3D.PlatformInfo.version)>=10,f=new XMLHttpRequest;f.open("GET",b,true);if(e)f.responseType="blob";else d?f.setRequestHeader("Accept-Charset","x-user-defined"):f.overrideMimeType("text/plain; charset=x-user-defined");f.onreadystatechange=function(){if(this.readyState==4)if(this.status==200||this.status==0){JSC3D.console&&JSC3D.console.logInfo('Finished loading STL file "'+b+'".');if(c.onload){c.onprogress&&c.onprogress("Loading STL file ...",
1);if(e){var g=new FileReader;g.onload=function(i){var l=new JSC3D.Scene;c.parseStl(l,i.target.result);c.onload(l)};g.readAsText(this.response,"x-user-defined")}else{if(d){g=new JSC3D.Scene;try{c.parseStl(g,function(i){for(var l="",m=0;m<i.length-65536;m+=65536)l+=String.fromCharCode.apply(null,i.slice(m,m+65536));return l+String.fromCharCode.apply(null,i.slice(m))}((new VBArray(this.responseBody)).toArray()))}catch(h){}}else{g=new JSC3D.Scene;c.parseStl(g,this.responseText)}c.onload(g)}}}else{JSC3D.console&&
JSC3D.console.logError('Failed to load STL file "'+b+'".');c.onerror&&c.onerror('Failed to load STL file "'+b+'".')}};if(this.onprogress){this.onprogress("Loading STL file ...",0);f.onprogress=function(g){c.onprogress("Loading STL file ...",g.position/g.totalSize)}}f.send()};a.setDecimalPrecision=function(b){this.decimalPrecision=b};
...
e;h={};for(g=0;g<e;g++){d.faceNormalBuffer.push(f.readFloat32());d.faceNormalBuffer.push(f.readFloat32());d.faceNormalBuffer.push(f.readFloat32());for(c=0;c<3;c++){i=f.readFloat32();l=f.readFloat32();m=f.readFloat32();u=i.toFixed(this.decimalPrecision)+"-"+l.toFixed(this.decimalPrecision)+"-"+m.toFixed(this.decimalPrecision);j=h[u];if(j==undefined){j=d.vertexBuffer.length/3;h[u]=j;d.vertexBuffer.push(i);d.vertexBuffer.push(l);d.vertexBuffer.push(m)}d.indexBuffer.push(j)}d.indexBuffer.push(-1);f.skip(2)}}else{f=
...
"ig");if(g=c.match(f)){e=g.length;d.faceCount=e;var h={};f.lastIndex=0;f.global=false;for(e=f.exec(c);e!=null;e=f.exec(c)){d.faceNormalBuffer.push(parseFloat(e[1]),parseFloat(e[2]),parseFloat(e[3]));for(g=0;g<3;g++){var i=parseFloat(e[4+g*3]),l=parseFloat(e[5+g*3]),m=parseFloat(e[6+g*3]),u=i.toFixed(this.decimalPrecision)+"-"+l.toFixed(this.decimalPrecision)+"-"+m.toFixed(this.decimalPrecision),j=h[u];if(j===undefined){j=d.vertexBuffer.length/3;h[u]=j;d.vertexBuffer.push(i);d.vertexBuffer.push(l);
d.vertexBuffer.push(m)}d.indexBuffer.push(j)}d.indexBuffer.push(-1)}}}if(!d.isTrivial()){if(Math.abs(d.faceNormalBuffer[0])<1.0E-6&&Math.abs(d.faceNormalBuffer[1])<1.0E-6&&Math.abs(d.faceNormalBuffer[2])<1.0E-6)d.faceNormalBuffer=null;b.addChild(d)}};a.onload=null;a.onerror=null;a.onprogress=null;a.onresource=null;a.decimalPrecision=3;JSC3D.LoaderSelector.registerLoader("stl",JSC3D.StlLoader);
