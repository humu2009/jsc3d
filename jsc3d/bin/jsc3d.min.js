...
this.mouseUsage="default";this.isDefaultInputHandlerEnabled=true;var e=this;if(JSC3D.PlatformInfo.isTouchDevice){this.canvas.addEventListener("touchstart",function(d){e.touchStartHandler(d)},false);this.canvas.addEventListener("touchend",function(d){e.touchEndHandler(d)},false);this.canvas.addEventListener("touchmove",function(d){e.touchMoveHandler(d)},false)}else{this.canvas.addEventListener("mousedown",function(d){e.mouseDownHandler(d)},false);this.canvas.addEventListener("mouseup",function(d){e.mouseUpHandler(d)},
false);this.canvas.addEventListener("mousemove",function(d){e.mouseMoveHandler(d)},false);document.addEventListener("keydown",function(d){e.keyDownHandler(d)},false);document.addEventListener("keyup",function(d){e.keyUpHandler(d)},false)}};a=JSC3D.Viewer.prototype;a.setParameter=function(b,c){this.params[b]=c};
...
a.pick=function(b,c){var e=new JSC3D.PickInfo,d=this.canvas.getBoundingClientRect();b=b-d.left;c=c-d.top;d=b;var f=c;if(this.selectionBuffer!=null&&b>=0&&b<this.canvas.width&&c>=0&&c<this.canvas.height){switch(this.definition){case "low":d=~~(d/2);f=~~(f/2);break;case "high":d*=2;f*=2;break;case "standard":default:break}var g=this.selectionBuffer[f*this.frameWidth+d];if(g>0)for(var h=this.scene.getChildren(),i=0;i<h.length;i++)if(h[i].internalId==g){e.mesh=h[i];break}}e.canvasX=b;e.canvasY=c;if(e.mesh)e.depth=
this.zBuffer[f*this.frameWidth+d];return e};a.doUpdate=function(){if(this.needUpdate||this.needRepaint){this.beforeupdate!=null&&typeof this.beforeupdate=="function"&&this.beforeupdate();if(this.scene){if(this.needUpdate&&this.colorBuffer!=null){this.beginScene();this.render();this.endScene()}this.paint()}else this.drawBackground();this.needUpdate=this.needRepaint=false;this.afterupdate!=null&&typeof this.afterupdate=="function"&&this.afterupdate()}};
...
this.mouseY)*360/this.canvas.width);this.rotMatrix.rotateAboutYAxis(c)}this.mouseX=b.clientX;this.mouseY=b.clientY;this.update()}}};a.touchStartHandler=function(b){if(b.touches.length>0){var c=b.touches[0].clientX,e=b.touches[0].clientY;if(this.onmousedown){var d=this.pick(c,e);this.onmousedown(d.canvasX,d.canvasY,0,d.depth,d.mesh)}b.preventDefault();if(this.isDefaultInputHandlerEnabled){this.buttonStates[0]=true;this.mouseX=c;this.mouseY=e}}};
...
a.touchMoveHandler=function(b){if(b.touches.length>0){var c=b.touches[0].clientX,e=b.touches[0].clientY;if(this.onmousemove){var d=this.pick(c,e);this.onmousemove(d.canvasX,d.canvasY,0,d.depth,d.mesh)}b.preventDefault();if(this.isDefaultInputHandlerEnabled){if(this.mouseUsage=="zoom")this.zoomFactor*=this.mouseY<=e?1.11:0.9;else if(this.mouseUsage=="default"||this.mouseUsage=="rotate"){b=(c-this.mouseX)*360/this.canvas.height;this.rotMatrix.rotateAboutXAxis((e-this.mouseY)*360/this.canvas.width);
this.rotMatrix.rotateAboutYAxis(b)}this.mouseX=c;this.mouseY=e;this.update()}}};a.keyDownHandler=function(b){if(this.isDefaultInputHandlerEnabled)this.keyStates[b.keyCode]=true};a.keyUpHandler=function(b){if(this.isDefaultInputHandlerEnabled)this.keyStates[b.keyCode]=false};
a.loadScene=function(){this.scene=null;this.isLoaded=false;if(this.sceneUrl=="")return false;var b=this.sceneUrl.lastIndexOf("/");if(b==-1)b=this.sceneUrl.lastIndexOf("\\");b=this.sceneUrl.substring(b+1);var c=b.lastIndexOf(".");if(c==-1)return false;b=b.substring(c+1);b=JSC3D.LoaderSelector.getLoader(b);if(!b)return false;var e=this;b.onload=function(d){e.setupScene(d)};b.onerror=function(d){e.scene=null;e.isLoaded=false;e.isFailed=true;e.errorMsg=d;e.update()};b.onprogress=function(d,f){e.reportProgress(d,
f)};b.onresource=function(d){d instanceof JSC3D.Texture&&e.isMipMappingOn&&!d.hasMipmap()&&d.generateMipmaps();e.update()};b.loadFromUrl(this.sceneUrl);return true};
a.setupScene=function(b){b.init();if(!b.isEmpty()){var c=b.aabb.lengthOfDiagonal(),e=this.frameWidth,d=this.frameHeight;this.zoomFactor=c==0?1:(e<d?e:d)/c}this.rotMatrix.identity();this.rotMatrix.rotateAboutXAxis(this.initRotX);this.rotMatrix.rotateAboutYAxis(this.initRotY);this.rotMatrix.rotateAboutZAxis(this.initRotZ);this.scene=b;this.isLoaded=true;this.isFailed=false;this.errorMsg="";this.needRepaint=this.needUpdate=false;this.update()};
a.reportProgress=function(b,c){if(this.ctx){this.drawBackground();this.ctx.save();var e=255-((this.bkgColor1&65280)>>8),d=255-(this.bkgColor1&255);e="#"+(255-((this.bkgColor1&16711680)>>16)).toString(16)+e.toString(16)+d.toString(16);this.ctx.strokeStyle=e;this.ctx.fillStyle=e;e=this.canvas.height*0.38;d=this.canvas.width-80;this.ctx.strokeRect(40,e,d,20);this.ctx.fillRect(42,e+2,(d-4)*c,16);this.ctx.font="12px Courier New";this.ctx.textAlign="left";this.ctx.fillText(b,40,e-4,d);this.ctx.restore()}};
a.reportError=function(b){if(this.ctx){this.drawBackground();this.ctx.save();var c=this.canvas.height*0.38-4,e=255-((this.bkgColor1&65280)>>8),d=255-(this.bkgColor1&255);this.ctx.fillStyle="#"+(255-((this.bkgColor1&16711680)>>16)).toString(16)+e.toString(16)+d.toString(16);this.ctx.font="16px Courier New";this.ctx.textAlign="left";this.ctx.fillText(b,40,c);this.ctx.restore()}};a.generateBackground=function(){this.bkgImage?this.fillBackgroundWithImage():this.fillGradientBackground()};
a.fillGradientBackground=function(){for(var b=this.frameWidth,c=this.frameHeight,e=this.bkgColorBuffer,d=(this.bkgColor1&16711680)>>16,f=(this.bkgColor1&65280)>>8,g=this.bkgColor1&255,h=(this.bkgColor2&16711680)>>16,i=(this.bkgColor2&65280)>>8,m=this.bkgColor2&255,o=0,r=0;r<c;r++)for(var j=d+r*(h-d)/c&255,B=f+r*(i-f)/c&255,k=g+r*(m-g)/c&255,M=0;M<b;M++)e[o++]=j<<16|B<<8|k};
...
a.endScene=function(){var b=this.canvasData.data,c=this.canvas.width,e=this.canvas.height,d=this.colorBuffer,f=this.frameWidth,g=f*this.frameHeight;switch(this.definition){case "low":var h=c>>1,i=f-h,m=0,o=0;for(g=0;g<e;g++){for(var r=0;r<c;r++){f=d[m];b[o]=(f&16711680)>>16;b[o+1]=(f&65280)>>8;b[o+2]=f&255;b[o+3]=255;m+=r&1;o+=4}m+=g&1?i:-h}break;case "high":for(g=o=m=0;g<e;g++){for(r=0;r<c;r++){h=d[m];i=d[m+1];var j=d[m+f],B=d[m+f+1];b[o]=(h&16711680)+(i&16711680)+(j&16711680)+(B&16711680)>>18;b[o+
1]=(h&65280)+(i&65280)+(j&65280)+(B&65280)>>10;b[o+2]=(h&255)+(i&255)+(j&255)+(B&255)>>2;b[o+3]=255;m+=2;o+=4}m+=f}break;case "standard":default:for(o=m=0;m<g;m++,o+=4){f=d[m];b[o]=(f&16711680)>>16;b[o+1]=(f&65280)>>8;b[o+2]=f&255;b[o+3]=255}break}};
...
a.renderPoint=function(b){var c=this.frameWidth,e=c-1,d=this.frameHeight-1,f=b.transformedVertexBuffer,g=b.transformedVertexNormalZBuffer,h=this.colorBuffer,i=this.zBuffer,m=this.selectionBuffer,o=f.length/3,r=b.internalId,j=b.material?b.material.diffuseColor:this.defaultMaterial.diffuseColor;if(!g||g.length<o){b.transformedVertexNormalZBuffer=new Array(o);g=b.transformedVertexNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.vertexNormalBuffer,g);for(var B=0,k=0;B<o;B++,k+=3){var M=g[B];
if(b.isDoubleSided)M=M>0?M:-M;if(M>0){var v=~~(f[k]+0.5),y=~~(f[k+1]+0.5);M=f[k+2];if(v>=0&&v<e&&y>=0&&y<d){v=y*c+v;if(M>i[v]){i[v]=M;h[v]=j;m[v]=r}v++;if(M>i[v]){i[v]=M;h[v]=j;m[v]=r}v+=e;if(M>i[v]){i[v]=M;h[v]=j;m[v]=r}v++;if(M>i[v]){i[v]=M;h[v]=j;m[v]=r}}}}};
a.renderWireframe=function(b){var c=this.frameWidth,e=c-1,d=this.frameHeight-1,f=b.indexBuffer,g=b.transformedVertexBuffer,h=b.transformedFaceNormalZBuffer,i=this.colorBuffer,m=this.zBuffer,o=this.selectionBuffer,r=b.faceCount,j=b.internalId,B=b.material?b.material.diffuseColor:this.defaultMaterial.diffuseColor;if(!h||h.length<r){b.transformedFaceNormalZBuffer=new Array(r);h=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,h);for(var k=0,M=0;k<r;){var v=
h[k++];if(b.isDoubleSided)v=v>0?v:-v;if(v<0){do;while(f[M++]!=-1)}else{var y,D;y=f[M++]*3;D=f[M++]*3;v=y;for(var I=false;!I;){var z=~~(g[y]+0.5),w=~~(g[y+1]+0.5),s=g[y+2],u=~~(g[D]+0.5),n=~~(g[D+1]+0.5),q=g[D+2],A=u-z,p=n-w,E=q-s,l,N,x;if(Math.abs(A)>Math.abs(p)){l=A;N=A>0?1:-1;x=A!=0?N*p/A:0;A=A!=0?N*E/A:0}else{l=p;x=p>0?1:-1;N=p!=0?x*A/p:0;A=p!=0?x*E/p:0}z=z;w=w;s=s;if(l<0){z=u;w=n;s=q;l=-l;N=-N;x=-x;A=-A}for(u=0;u<l;u++){if(z>=0&&z<e&&w>=0&&w<d){n=~~w*c+~~z;if(s>m[n]){m[n]=s;i[n]=B;o[n]=j}}z+=
N;w+=x;s+=A}if(D==v)I=true;else{y=D;D=f[M]!=-1?f[M++]*3:v}}M++}}};
a.renderSolidFlat=function(b){var c=this.frameWidth,e=this.frameHeight,d=b.indexBuffer,f=b.transformedVertexBuffer,g=b.transformedFaceNormalZBuffer,h=this.colorBuffer,i=this.zBuffer,m=this.selectionBuffer,o=b.faceCount,r=b.internalId,j=b.material?b.material:this.defaultMaterial,B=j.getPalette(),k=j.transparency==0,M=j.transparency*255,v=255-M;if(j.transparency!=1){if(!g||g.length<o){b.transformedFaceNormalZBuffer=new Array(o);g=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,
g);j=new Array(3);for(var y=new Array(3),D=new Array(3),I=0,z=0;I<o;){var w=g[I++];if(b.isDoubleSided)w=w>0?w:-w;if(w<0){do;while(d[z++]!=-1)}else{w=B[~~(w*255)];var s,u,n;s=d[z++]*3;u=d[z++]*3;do{n=d[z++]*3;j[0]=~~(f[s]+0.5);y[0]=~~(f[s+1]+0.5);D[0]=f[s+2];j[1]=~~(f[u]+0.5);y[1]=~~(f[u+1]+0.5);D[1]=f[u+2];j[2]=~~(f[n]+0.5);y[2]=~~(f[n+1]+0.5);D[2]=f[n+2];u=y[0]<y[1]?0:1;u=y[u]<y[2]?u:2;var q=y[0]>y[1]?0:1;q=y[q]>y[2]?q:2;var A=3-q-u;if(u!=q){var p=j[q],E=D[q],l=y[q]-y[u];l=l!=0?l:1;var N=(j[q]-j[u])/
l;l=(D[q]-D[u])/l;var x=j[q],t=D[q],C=y[q]-y[A];C=C!=0?C:1;var J=(j[q]-j[A])/C;C=(D[q]-D[A])/C;var R=j[A],O=D[A],F=y[A]-y[u];F=F!=0?F:1;var ha=(j[A]-j[u])/F;F=(D[A]-D[u])/F;var X=y[q]*c;for(q=y[q];q>y[u];q--){if(q>=0&&q<e){var $=~~p,G=E,aa,oa;if(q>y[A]){aa=~~x;oa=t}else{aa=~~R;oa=O}if($>aa){var ca;ca=$;$=aa;aa=ca;ca=G;G=oa;oa=ca}if($<0)$=0;if(aa>=c)aa=c-1;oa=$!=aa?(oa-G)/(aa-$):1;ca=X+$;if(k){$=$;for(G=G;$<=aa;$++,G+=oa){if(G>i[ca]){i[ca]=G;h[ca]=w;m[ca]=r}ca++}}else{$=$;for(G=G;$<aa;$++,G+=oa){if(G>
i[ca]){var ma=h[ca];h[ca]=(ma&16711680)*M+(w&16711680)*v>>8&16711680|(ma&65280)*M+(w&65280)*v>>8&65280|(ma&255)*M+(w&255)*v>>8&255;m[ca]=r}ca++}}}p-=N;E-=l;if(q>y[A]){x-=J;t-=C}else{R-=ha;O-=F}X-=c}}u=n}while(d[z]!=-1);z++}}}};
a.renderSolidSmooth=function(b){var c=this.frameWidth,e=this.frameHeight,d=b.indexBuffer,f=b.transformedVertexBuffer,g=b.transformedVertexNormalZBuffer,h=b.transformedFaceNormalZBuffer,i=this.colorBuffer,m=this.zBuffer,o=this.selectionBuffer,r=b.faceCount,j=f.length/3,B=b.internalId,k=b.material?b.material:this.defaultMaterial,M=k.getPalette(),v=k.transparency==0,y=k.transparency*255,D=255-y;if(k.transparency!=1){if(!g||g.length<j){b.transformedVertexNormalZBuffer=new Array(j);g=b.transformedVertexNormalZBuffer}if(!h||
h.length<r){b.transformedFaceNormalZBuffer=new Array(r);h=b.transformedFaceNormalZBuffer}JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.vertexNormalBuffer,g);JSC3D.Math3D.transformVectorZs(this.rotMatrix,b.faceNormalBuffer,h);b=b.isDoubleSided;j=new Array(3);k=new Array(3);for(var I=new Array(3),z=new Array(3),w=0,s=0;w<r;){var u=h[w++];if(b)u=u>0?u:-u;if(u<0){do;while(d[s++]!=-1)}else{var n,q,A,p,E;u=d[s++];A=u*3;n=d[s++];p=n*3;do{q=d[s++];E=q*3;j[0]=~~(f[A]+0.5);k[0]=~~(f[A+1]+0.5);I[0]=f[A+2];
...
var aa=j[p],oa=I[p],ca=z[p]*255,ma=k[p]-k[n];ma=ma!=0?ma:1;var ra=(j[p]-j[n])/ma,xa=(I[p]-I[n])/ma;ma=(z[p]-z[n])*255/ma;var ta=k[l]*c;for(l=k[l];l>k[n];l--){if(l>=0&&l<e){var da=~~N,ea=x,ka=t,na,ua,sa;if(l>k[p]){na=~~O;ua=F;sa=ha}else{na=~~aa;ua=oa;sa=ca}if(da>na){var Y;Y=da;da=na;na=Y;Y=ea;ea=ua;ua=Y;Y=ka;ka=sa;sa=Y}ua=da!=na?(ua-ea)/(na-da):1;sa=da!=na?(sa-ka)/(na-da):1;if(da<0){ea-=da*ua;ka-=da*sa;da=0}if(na>=c)na=c-1;Y=ta+da;if(v){da=da;ea=ea;for(ka=ka;da<=na;da++,ea+=ua,ka+=sa){if(ea>m[Y]){m[Y]=
ea;i[Y]=M[ka>0?~~ka:0];o[Y]=B}Y++}}else{da=da;ea=ea;for(ka=ka;da<na;da++,ea+=ua,ka+=sa){if(ea>m[Y]){var pa=M[ka>0?~~ka:0],za=i[Y];i[Y]=(za&16711680)*y+(pa&16711680)*D>>8&16711680|(za&65280)*y+(pa&65280)*D>>8&65280|(za&255)*y+(pa&255)*D>>8&255;o[Y]=B}Y++}}}N-=J;x-=R;t-=C;if(l>k[p]){O-=$;F-=G;ha-=X}else{aa-=ra;oa-=xa;ca-=ma}ta-=c}}p=E;n=q}while(d[s]!=-1);s++}}}};
...
b.faceNormalBuffer,g);for(var w=new Array(3),s=new Array(3),u=new Array(3),n=new Array(3),q=new Array(3),A=0,p=0;A<o;){var E=g[A++];if(b.isDoubleSided)E=E>0?E:-E;if(E<0){do;while(d[p++]!=-1)}else{var l,N,x,t,C;E=d[p]*3;x=M[p]*2;p++;l=d[p]*3;t=M[p]*2;p++;if(I){N=d[p]*3;C=M[p]*2;y=j.width;w[0]=f[E];s[0]=f[E+1];w[1]=f[l];s[1]=f[l+1];w[2]=f[N];s[2]=f[N+1];n[0]=k[x]*y;q[0]=k[x+1]*y;n[1]=k[t]*y;q[1]=k[t+1]*y;n[2]=k[C]*y;q[2]=k[C+1]*y;v=(w[1]-w[0])*(s[2]-s[0])-(s[1]-s[0])*(w[2]-w[0]);if(v<0)v=-v;v+=1;D=
(n[1]-n[0])*(q[2]-q[0])-(q[1]-q[0])*(n[2]-n[0]);if(D<0)D=-D;v=D/v;D=0;if(v<z[1])D=0;else if(v>=z[z.length-1]){D=z.length-1;y=1}else for(;v>=z[D+1];){D++;y/=2}v=I[D];D=y-1}do{N=d[p]*3;C=M[p]*2;p++;w[0]=~~(f[E]+0.5);s[0]=~~(f[E+1]+0.5);u[0]=f[E+2];w[1]=~~(f[l]+0.5);s[1]=~~(f[l+1]+0.5);u[1]=f[l+2];w[2]=~~(f[N]+0.5);s[2]=~~(f[N+1]+0.5);u[2]=f[N+2];n[0]=k[x]*y;q[0]=k[x+1]*y;n[1]=k[t]*y;q[1]=k[t+1]*y;n[2]=k[C]*y;q[2]=k[C+1]*y;l=s[0]<s[1]?0:1;l=s[l]<s[2]?l:2;var J=s[0]>s[1]?0:1;J=s[J]>s[2]?J:2;t=3-J-l;if(l!=
...
(Z&16711680)*V>>8&16711680|(ja&65280)*L+(Z&65280)*V>>8&65280|(ja&255)*L+(Z&255)*V>>8&255;m[P]=r}P++}}}R-=$;O-=G;F-=aa;ha-=X;if(J>s[t]){oa-=ta;ca-=da;ma-=ea;ra-=xa}else{ka-=pa;na-=za;ua-=la;sa-=Y}Ea-=c}}l=N;t=C}while(d[p]!=-1);p++}}};
...
X-=ca;$-=ma;G-=aa;if(F>n[R]){ra-=ka;xa-=na;ta-=ua;da-=ea}else{sa-=Ea;Y-=fa;pa-=va;za-=la}wa-=c}}t=C;R=O}while(d[l]!=-1);l++}}}};
...
N=0,x=0;N<r;){var t=h[N++];if(b)t=t>0?t:-t;if(t<0){do;while(d[x++]!=-1)}else{var C,J,R,O,F,ha,X,$;t=d[x];R=t*3;ha=z[x]*2;x++;C=d[x];O=C*3;X=z[x]*2;x++;if(n){F=d[x]*3;$=z[x]*2;s=v.width;B[0]=f[R];k[0]=f[R+1];B[1]=f[O];k[1]=f[O+1];B[2]=f[F];k[2]=f[F+1];E[0]=I[ha]*s;l[0]=I[ha+1]*s;E[1]=I[X]*s;l[1]=I[X+1]*s;E[2]=I[$]*s;l[2]=I[$+1]*s;w=(B[1]-B[0])*(k[2]-k[0])-(k[1]-k[0])*(B[2]-B[0]);if(w<0)w=-w;w+=1;u=(E[1]-E[0])*(l[2]-l[0])-(l[1]-l[0])*(E[2]-E[0]);if(u<0)u=-u;w=u/w;u=0;if(w<q[1])u=0;else if(w>=q[q.length-
1]){u=q.length-1;s=1}else for(;w>=q[u+1];){u++;s/=2}w=n[u];u=s-1}do{J=d[x];F=J*3;$=z[x]*2;x++;B[0]=~~(f[R]+0.5);k[0]=~~(f[R+1]+0.5);A[0]=f[R+2];B[1]=~~(f[O]+0.5);k[1]=~~(f[O+1]+0.5);A[1]=f[O+2];B[2]=~~(f[F]+0.5);k[2]=~~(f[F+1]+0.5);A[2]=f[F+2];E[0]=I[ha]*s;l[0]=I[ha+1]*s;E[1]=I[X]*s;l[1]=I[X+1]*s;E[2]=I[$]*s;l[2]=I[$+1]*s;p[0]=g[t];p[1]=g[C];p[2]=g[J];if(b){if(p[0]<0)p[0]=-p[0];if(p[1]<0)p[1]=-p[1];if(p[2]<0)p[2]=-p[2]}C=k[0]<k[1]?0:1;C=k[C]<k[2]?C:2;var G=k[0]>k[1]?0:1;G=k[G]>k[2]?G:2;O=3-G-C;if(C!=
...
2*v/la;la=(n[l]-n[t])/2*v/la;var Q=k[x]*c;for(x=k[x];x>k[l];x--){if(x>=0&&x<e){var H=~~C,U=J,K=R,S=O,P=F,Z,ja,V,L,ba;if(x>k[t]){Z=~~oa;ja=ca;V=ma;L=ra;ba=xa}else{Z=~~ua;ja=sa;V=Y;L=pa;ba=za}if(H>Z){var T;T=H;H=Z;Z=T;T=U;U=ja;ja=T;T=K;K=V;V=T;T=S;S=L;L=T;T=P;P=ba;ba=T}ja=H!=Z?(ja-U)/(Z-H):1;V=H!=Z?(V-K)/(Z-H):1;L=H!=Z?(L-S)/(Z-H):1;ba=H!=Z?(ba-P)/(Z-H):1;if(H<0){U-=H*ja;K-=H*V;S-=S*L;P-=P*ba;H=0}if(Z>=c)Z=c-1;T=Q+H;if(I){H=H;U=U;K=K;S=S;for(P=P;H<=Z;H++,U+=ja,K+=V,S+=L,P+=ba){if(U>m[T]){m[T]=U;var W=
...
X;J-=$;R-=G;O-=aa;F-=ha;if(x>k[t]){oa-=da;ca-=ea;ma-=ka;ra-=na;xa-=ta}else{ua-=Ea;sa-=fa;Y-=va;pa-=wa;za-=la}Q-=c}}l=N}while(d[p]!=-1);p++}}}};a.params=null;a.canvas=null;a.ctx=null;a.canvasData=null;a.bkgColorBuffer=null;a.colorBuffer=null;a.zBuffer=null;a.selectionBuffer=null;a.frameWidth=0;a.frameHeight=0;a.scene=null;a.defaultMaterial=null;a.sphereMap=null;a.isLoaded=false;a.isFailed=false;a.errorMsg="";a.needUpdate=false;a.needRepaint=false;a.initRotX=0;a.initRotY=0;a.initRotZ=0;
...
JSC3D.Mesh=function(b,c,e,d,f,g,h,i,m,o){this.name=b||"";this.metadata="";this.visible=c!=undefined?c:true;this.aabb=null;this.vertexBuffer=h||null;this.indexBuffer=i||null;this.faceNormalBuffer=this.vertexNormalBuffer=null;this.material=e||null;this.texture=d||null;this.faceCount=0;this.isDoubleSided=f||false;this.isEnvironmentCast=g||false;this.internalId=0;this.texCoordBuffer=m||null;this.texCoordIndexBuffer=o||null;this.transformedVertexNormalBuffer=this.transformedFaceNormalZBuffer=this.transformedVertexNormalZBuffer=
...
a.createFromImage=function(b,c){if(!(b.width<=0||b.height<=0)){var e=false,d=JSC3D.Texture.cv;if(!d)try{d=document.createElement("canvas");JSC3D.Texture.cv=d;e=true}catch(f){return}var g=b.width>b.height?b.width:b.height;g=g<=32?32:g<=64?64:g<=128?128:g<=256?256:512;if(d.width!=g||d.height!=g){d.width=d.height=g;e=true}var h;try{var i=d.getContext("2d");e||i.clearRect(0,0,g,g);i.drawImage(b,0,0,g,g);h=i.getImageData(0,0,g,g).data}catch(m){return}e=h.length/4;this.data=new Array(e);for(var o=i=0;i<
e;i++,o+=4){d=h[o+3];this.data[i]=d<<24|h[o]<<16|h[o+1]<<8|h[o+2];if(d<255)this.hasTransparency=true}this.height=this.width=g;this.mipmaps=null;c&&this.generateMipmaps();this.srcUrl=b.src;this.onready!=null&&typeof this.onready=="function"&&this.onready()}};a.hasData=function(){return this.data!=null};
a.generateMipmaps=function(){if(!(this.width<=1||this.data==null||this.mipmaps!=null)){this.mipmaps=[this.data];this.mipentries=[1];for(var b=1+~~(0.1+Math.log(this.width)*Math.LOG2E),c=this.width>>1,e=1;e<b;e++){for(var d=new Array(c*c),f=this.mipmaps[e-1],g=c<<1,h=0,i=0,m=0;m<c;m++){for(var o=0;o<c;o++){var r=f[h],j=f[h+1],B=f[h+g],k=f[h+g+1];d[i]=(((r&4278190080)>>>2)+((j&4278190080)>>>2)+((B&4278190080)>>>2)+((k&4278190080)>>>2)&4278190080)+((r&16711680)+(j&16711680)+(B&16711680)+(k&16711680)>>
2&16711680)+((r&65280)+(j&65280)+(B&65280)+(k&65280)>>2&65280)+((r&255)+(j&255)+(B&255)+(k&255)>>2&255);h+=2;i++}h+=g}this.mipmaps.push(d);this.mipentries.push(Math.pow(4,e));c>>=1}}};a.hasMipmap=function(){return this.mipmaps!=null};a.name="";a.data=null;a.mipmaps=null;a.mipentries=null;a.width=0;a.height=0;a.hasTransparency=false;a.srcUrl="";a.onready=null;JSC3D.Texture.cv=null;JSC3D.AABB=function(){this.minZ=this.maxZ=this.minY=this.maxY=this.minX=this.maxX=0};
...
a.rotateAboutXAxis=function(b){if(b!=0){b*=Math.PI/180;var c=Math.cos(b);b=Math.sin(b);var e=c*this.m11-b*this.m21,d=c*this.m12-b*this.m22,f=c*this.m13-b*this.m23,g=c*this.m20+b*this.m10,h=c*this.m21+b*this.m11,i=c*this.m22+b*this.m12,m=c*this.m23+b*this.m13;this.m10=c*this.m10-b*this.m20;this.m11=e;this.m12=d;this.m13=f;this.m20=g;this.m21=h;this.m22=i;this.m23=m}};
a.rotateAboutYAxis=function(b){if(b!=0){b*=Math.PI/180;var c=Math.cos(b);b=Math.sin(b);var e=c*this.m01+b*this.m21,d=c*this.m02+b*this.m22,f=c*this.m03+b*this.m23,g=c*this.m20-b*this.m00,h=c*this.m21-b*this.m01,i=c*this.m22-b*this.m02,m=c*this.m23-b*this.m03;this.m00=c*this.m00+b*this.m20;this.m01=e;this.m02=d;this.m03=f;this.m20=g;this.m21=h;this.m22=i;this.m23=m}};
a.rotateAboutZAxis=function(b){if(b!=0){b*=Math.PI/180;var c=Math.cos(b);b=Math.sin(b);var e=c*this.m10+b*this.m00,d=c*this.m11+b*this.m01,f=c*this.m12+b*this.m02,g=c*this.m13+b*this.m03,h=c*this.m01-b*this.m11,i=c*this.m02-b*this.m12,m=c*this.m03-b*this.m13;this.m00=c*this.m00-b*this.m10;this.m01=h;this.m02=i;this.m03=m;this.m10=e;this.m11=d;this.m12=f;this.m13=g}};
a.multiply=function(b){var c=b.m00*this.m01+b.m01*this.m11+b.m02*this.m21,e=b.m00*this.m02+b.m01*this.m12+b.m02*this.m22,d=b.m00*this.m03+b.m01*this.m13+b.m02*this.m23+b.m03,f=b.m10*this.m00+b.m11*this.m10+b.m12*this.m20,g=b.m10*this.m01+b.m11*this.m11+b.m12*this.m21,h=b.m10*this.m02+b.m11*this.m12+b.m12*this.m22,i=b.m10*this.m03+b.m11*this.m13+b.m12*this.m23+b.m13,m=b.m20*this.m00+b.m21*this.m10+b.m22*this.m20,o=b.m20*this.m01+b.m21*this.m11+b.m22*this.m21,r=b.m20*this.m02+b.m21*this.m12+b.m22*this.m22,
j=b.m20*this.m03+b.m21*this.m13+b.m22*this.m23+b.m23;this.m00=b.m00*this.m00+b.m01*this.m10+b.m02*this.m20;this.m01=c;this.m02=e;this.m03=d;this.m10=f;this.m11=g;this.m12=h;this.m13=i;this.m20=m;this.m21=o;this.m22=r;this.m23=j};
JSC3D.Math3D={transformVectors:function(b,c,e){for(var d=0;d<c.length;d+=3){var f=c[d],g=c[d+1],h=c[d+2];e[d]=b.m00*f+b.m01*g+b.m02*h+b.m03;e[d+1]=b.m10*f+b.m11*g+b.m12*h+b.m13;e[d+2]=b.m20*f+b.m21*g+b.m22*h+b.m23}},transformVectorZs:function(b,c,e){for(var d=c.length/3,f=0,g=0;f<d;){e[f]=b.m20*c[g]+b.m21*c[g+1]+b.m22*c[g+2]+b.m23;f++;g+=3}}};
JSC3D.PlatformInfo=function(){for(var b={browser:"other",version:"0.0.0",isTouchDevice:document.createTouch!=undefined},c=[["firefox",/Firefox[\/\s](\d+(?:.\d+)*)/],["chrome",/Chrome[\/\s](\d+(?:.\d+)*)/],["opera",/Opera[\/\s](\d+(?:.\d+)*)/],["safari",/Safari[\/\s](\d+(?:.\d+)*)/],["webkit",/AppleWebKit[\/\s](\d+(?:.\d+)*)/],["ie",/MSIE[\/\s](\d+(?:.\d+)*)/]],e,d=0;d<c.length;d++)if(e=c[d][1].exec(window.navigator.userAgent)){b.browser=c[d][0];b.version=e[1];break}return b}();
...
a.readFloat64=function(){return this.decodeFloat(8,52)};a.readBytes=function(b,c){var e=c;if(this.offset+c>this.data.length)e=this.data.length-this.offset;for(c=0;c<e;c++)b[c]=this.data[this.offset++].charCodeAt(0)&255;return e};a.decodeInt=function(b,c){if(this.offset+b>this.data.length){this.offset=this.data.length;return NaN}for(var e=0,d=1,f=0;f<b;f++){e+=(this.data[this.offset++].charCodeAt(0)&255)*d;d*=256}if(c&&e&Math.pow(2,b*8-1))e-=Math.pow(2,b*8);return e};
a.decodeFloat=function(b,c){if(this.offset+b>this.data.length){this.offset=this.data.length;return NaN}var e=b*8-c-1,d=(1<<e)-1,f=d>>1,g=b-1,h=this.data[this.offset+g].charCodeAt(0)&255;g+=-1;var i=-7,m=h&(1<<-i)-1;h>>=-i;for(i+=e;i>0;){m=m*256+(this.data[this.offset+g].charCodeAt(0)&255);g+=-1;i-=8}e=m&(1<<-i)-1;m>>=-i;for(i+=c;i>0;){e=e*256+(this.data[this.offset+g].charCodeAt(0)&255);g+=-1;i-=8}this.offset+=b;switch(m){case 0:m=1-f;break;case d:return e?NaN:(h?-1:1)*Infinity;default:e+=Math.pow(2,
c);m-=f;break}return(h?-1:1)*e*Math.pow(2,m-c)};JSC3D.LoaderSelector={registerLoader:function(b,c){if(typeof c=="function")JSC3D.LoaderSelector.loaderTable[b]=c},getLoader:function(b){b=JSC3D.LoaderSelector.loaderTable[b.toLowerCase()];if(!b)return null;var c;try{c=new b}catch(e){c=null}return c},loaderTable:{}};
JSC3D.ObjLoader=function(b,c,e,d){this.onload=b&&typeof b=="function"?b:null;this.onerror=c&&typeof c=="function"?c:null;this.onprogress=e&&typeof e=="function"?e:null;this.onresource=d&&typeof d=="function"?d:null;this.requestCount=0};a=JSC3D.ObjLoader.prototype;a.loadFromUrl=function(b){var c="",e=b,d=b.lastIndexOf("/");if(d==-1)d=b.lastIndexOf("\\");if(d!=-1){c=b.substring(0,d+1);e=b.substring(d+1)}this.requestCount=0;this.loadObjFile(c,e)};
a.loadObjFile=function(b,c){var e=b+c,d=this;c=new XMLHttpRequest;c.open("GET",e,true);c.onreadystatechange=function(){if(this.readyState==4)if(this.status==200||this.status==0){if(d.onload){d.onprogress&&d.onprogress("Loading obj file ...",1);JSC3D.console&&JSC3D.console.logInfo('Finished loading obj file "'+e+'".');var f=new JSC3D.Scene,g=d.parseObj(f,this.responseText);if(g.length>0)for(var h=0;h<g.length;h++)d.loadMtlFile(f,b,g[h]);--d.requestCount==0&&d.onload(f)}}else{JSC3D.console&&JSC3D.console.logError('Failed to load obj file "'+
e+'".');if(d.onerror){d.requestCount--;d.onerror('Failed to load obj file "'+e+'".')}}};if(this.onprogress){this.onprogress("Loading obj file ...",0);c.onprogress=function(f){d.onprogress("Loading obj file ...",f.position/f.totalSize)}}this.requestCount++;c.send()};
a.loadMtlFile=function(b,c,e){var d=c+e,f=this,g=new XMLHttpRequest;g.open("GET",d,true);g.onreadystatechange=function(){if(this.readyState==4){if(this.status==200||this.status==0){f.onprogress&&f.onprogress("Loading mtl file ...",1);JSC3D.console&&JSC3D.console.logInfo('Finished loading mtl file "'+d+'".');for(var h=f.parseMtl(this.responseText),i={},m=b.getChildren(),o=0;o<m.length;o++){var r=m[o];if(r.mtl!=null&&r.mtllib!=null&&r.mtllib==e){var j=h[r.mtl];if(j!=null){j.material!=null&&r.setMaterial(j.material);
if(j.textureFileName!="")if(i[j.textureFileName])i[j.textureFileName].push(r);else i[j.textureFileName]=[r]}}}for(var B in i)f.setupTexture(i[B],c+B)}else JSC3D.console&&JSC3D.console.logWarning('Failed to load mtl file "'+d+'". A default material will be applied.');--f.requestCount==0&&f.onload(b)}};if(this.onprogress){this.onprogress("Loading mtl file ...",0);g.onprogress=function(h){f.onprogress("Loading mtl file ...",h.position/h.totalSize)}}this.requestCount++;g.send()};
a.parseObj=function(b,c){var e={},d=[],f=0,g=null,h="",i="",m=[],o=[];g="obj-"+f++;var r=new JSC3D.Mesh;r.name=g;r.indexBuffer=[];g=e.nomtl=r;var j=c.split(/[ \t]*\r?\n[ \t]*/);for(c=0;c<j.length;c++){i=j[c].split(/[ \t]+/);if(i.length>0)switch(i[0]){case "v":if(i.length>3)for(var B=1;B<4;B++)m.push(parseFloat(i[B]));break;case "vn":break;case "vt":if(i.length>2){o.push(parseFloat(i[1]));o.push(1-parseFloat(i[2]))}break;case "f":if(i.length>3){for(B=1;B<i.length;B++){var k=i[B].split("/");g.indexBuffer.push(parseInt(k[0])-
1);if(k.length>1&&k[1]!=""){if(!g.texCoordIndexBuffer)g.texCoordIndexBuffer=[];g.texCoordIndexBuffer.push(parseInt(k[1])-1)}}g.indexBuffer.push(-1);g.texCoordIndexBuffer&&g.texCoordIndexBuffer.push(-1)}break;case "mtllib":if(i.length>1){h=i[1];d.push(h)}else h="";break;case "usemtl":if(i.length>1&&i[1]!=""&&h!=""){i=i[1];B=h+"-"+i;g=e[B];if(!g){g=new JSC3D.Mesh;g.name="obj-"+f++;g.indexBuffer=[];g.mtllib=h;g.mtl=i;e[B]=g}g=g}else g=r;break;case "#":default:break}}f=m.length>=3?new Array(m.length/
3):null;h=o.length>=2?new Array(o.length/2):null;for(var M in e){g=e[M];if(m.length>=3&&g.indexBuffer.length>0){for(c=0;c<f.length;c++)f[c]=-1;g.vertexBuffer=[];for(c=j=r=0;c<g.indexBuffer.length;c++){r=g.indexBuffer[c];if(r!=-1)if(f[r]==-1){i=r*3;g.vertexBuffer.push(m[i]);g.vertexBuffer.push(m[i+1]);g.vertexBuffer.push(m[i+2]);g.indexBuffer[c]=j;f[r]=j;j++}else g.indexBuffer[c]=f[r]}}if(o.length>=2&&g.texCoordIndexBuffer!=null&&g.texCoordIndexBuffer.length>0){for(c=0;c<h.length;c++)h[c]=-1;g.texCoordBuffer=
[];for(c=j=r=0;c<g.texCoordIndexBuffer.length;c++){r=g.texCoordIndexBuffer[c];if(r!=-1)if(h[r]==-1){i=r*2;g.texCoordBuffer.push(o[i]);g.texCoordBuffer.push(o[i+1]);g.texCoordIndexBuffer[c]=j;h[r]=j;j++}else g.texCoordIndexBuffer[c]=h[r]}}g.isTrivial()||b.addChild(g)}return d};
...
JSC3D.StlLoader=function(b,c,e,d){this.onload=b&&typeof b=="function"?b:null;this.onerror=c&&typeof c=="function"?c:null;this.onprogress=e&&typeof e=="function"?e:null;this.onresource=d&&typeof d=="function"?d:null;this.decimalPrecision=3};a=JSC3D.StlLoader.prototype;
a.loadFromUrl=function(b){var c=this,e=new XMLHttpRequest;e.open("GET",b,true);if(JSC3D.PlatformInfo.browser=="ie"&&JSC3D.PlatformInfo.version>="10")e.responseType="blob";else e.overrideMimeType("text/plain; charset=x-user-defined");e.onreadystatechange=function(){if(this.readyState==4)if(this.status==200||this.status==0){JSC3D.console&&JSC3D.console.logInfo('Finished loading STL file "'+b+'".');if(c.onload){c.onprogress&&c.onprogress("Loading STL file ...",1);if(JSC3D.PlatformInfo.browser=="ie"&&
JSC3D.PlatformInfo.version>="10"){var d=new FileReader;d.onload=function(f){var g=new JSC3D.Scene;c.parseStl(g,f.target.result);c.onload(g)};d.readAsText(this.response,"x-user-defined")}else{d=new JSC3D.Scene;c.parseStl(d,this.responseText);c.onload(d)}}}else{JSC3D.console&&JSC3D.console.logError('Failed to load STL file "'+b+'".');c.onerror&&c.onerror('Failed to load STL file "'+b+'".')}};if(this.onprogress){this.onprogress("Loading STL file ...",0);e.onprogress=function(d){c.onprogress("Loading STL file ...",
d.position/d.totalSize)}}e.send()};a.setDecimalPrecision=function(b){this.decimalPrecision=b};
...
d;h={};for(g=0;g<d;g++){e.faceNormalBuffer.push(f.readFloat32());e.faceNormalBuffer.push(f.readFloat32());e.faceNormalBuffer.push(f.readFloat32());for(c=0;c<3;c++){i=f.readFloat32();m=f.readFloat32();o=f.readFloat32();r=i.toFixed(this.decimalPrecision)+"-"+m.toFixed(this.decimalPrecision)+"-"+o.toFixed(this.decimalPrecision);j=h[r];if(j==undefined){j=e.vertexBuffer.length/3;h[r]=j;e.vertexBuffer.push(i);e.vertexBuffer.push(m);e.vertexBuffer.push(o)}e.indexBuffer.push(j)}e.indexBuffer.push(-1);f.skip(2)}}else{f=
...
"ig");if(g=c.match(f)){d=g.length;e.faceCount=d;var h={};f.lastIndex=0;f.global=false;for(d=f.exec(c);d!=null;d=f.exec(c)){e.faceNormalBuffer.push(parseFloat(d[1]),parseFloat(d[2]),parseFloat(d[3]));for(g=0;g<3;g++){var i=parseFloat(d[4+g*3]),m=parseFloat(d[5+g*3]),o=parseFloat(d[6+g*3]),r=i.toFixed(this.decimalPrecision)+"-"+m.toFixed(this.decimalPrecision)+"-"+o.toFixed(this.decimalPrecision),j=h[r];if(j===undefined){j=e.vertexBuffer.length/3;h[r]=j;e.vertexBuffer.push(i);e.vertexBuffer.push(m);
e.vertexBuffer.push(o)}e.indexBuffer.push(j)}e.indexBuffer.push(-1)}}}e.isTrivial()||b.addChild(e)};a.onload=null;a.onerror=null;a.onprogress=null;a.onresource=null;a.decimalPrecision=3;JSC3D.LoaderSelector.registerLoader("stl",JSC3D.StlLoader);
